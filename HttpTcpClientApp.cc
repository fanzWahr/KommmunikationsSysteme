...
void HttpTcpClientApp::handleMessage(cMessage *msg) {
    ...
    if (msg == dataProcessing) {
        ...
        // Der Socket entscheidet ��ber den Aufruf von "socketX"-Methode
...
void HttpTcpClientApp::socketDataIndication(int connId, void *ptr) {
    EV << "TCP m��chte Daten an Anwendung ��bergeben\n";
    ...
    if (!processing) {
        // Nein, also Datenempfang einleiten, Daten werden nun per Message ��bergeben
        socket.receive();
    }
}
...
void HttpTcpClientApp::socketDataArrived(int connId, void *ptr, cPacket *msg,
    ...
    if (body.length() > 0) {
        // Empfang beendet
        EV << "HTTP-Body: L��nge: " << body.length() << "\nHTTP:Body (max. 100 Zeichen);\n" << body.substr(0, min((int)body.size()-1, 100)) << endl;
        cout << "HTTP-Body: L��nge: " << body.length() << "\nHTTP:Body (max. 100 Zeichen);\n" << body.substr(0, min((int)body.size()-1, 100)) << endl;
        socket.close();
    } else {
        // weiter empfangen
        if (processing) {
            // Verarbeitungszeit der Anwendung verl��ngern, dazu laufenden Timeout beenden
