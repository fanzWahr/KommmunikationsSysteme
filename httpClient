...
long contentLength; /**< L��nge des gesamten bodies der HTTP-Nachricht */
...
void prepareRequest(string httpRequest, RawPacket *packet) {
    ...
    // HTTP-Request zum ��bertragen vorbereiten
...
string handleReply(char *resp) { //TODO : Fuer den Gebrauch an TCP anpassen!
    // Diese Methode funktioniert nur, wenn die gesammte HTTP-Nachricht auf ein Mal empfangen wurde!
    // Es ist zu beachten, das die HTTP-Nachricht in Fragmenten empfangen werden k��nnte!
    ...
    /* ??? So erh��lt man z.B. die L��nge der HTTP-Nachricht! ??? */
    contentLength = stol(header["Content-Length"]);

    // HTTP-Body zur��ckliefern
    return body;
}
...
void parseHttp(string message, map<string, string> &headerMap, string &body) {
    ...
    // die restlichen HTTP-Header-Eintr��ge auswerten
